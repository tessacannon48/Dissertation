...
class ConditionalUNet(nn.Module):
    ...
    """
    def __init__(self, in_channels=1, cond_channels=24, attr_dim=48,
                 base_channels=128, embed_dim=256, 
                 unet_depth=4, attention_variant='default'):
        super().__init__()
        ...
        self.attr_mlp = nn.Sequential(
            nn.Linear(attr_dim, embed_dim), nn.SiLU(),
            nn.Linear(embed_dim, embed_dim)
        )
        ...
        for i in range(unet_depth):
            ...
            out_ch = skip_ch  # reduce to encoder's output channels
            use_attn = self._use_attention(i, stage='up', depth=unet_depth)
            self.ups.append(Up(decoder_channels=in_ch, skip_channels=skip_ch,
                            out_channels=out_ch, embed_dim=embed_dim, use_attention=use_attn))
            in_ch = out_ch
    ...
    def _use_attention(self, idx, stage, depth):
        ...
        if self.attention_variant == 'none':
            ...
            # mimic your original setup (attn in down1/down2/up1/up2)
